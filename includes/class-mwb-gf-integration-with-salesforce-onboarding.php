 ...
 */
if ( class_exists( 'Mwb_Gf_Integration_With_Salesforce_Onboarding' ) ) {
	return;
}
 ...
 */
class Mwb_Gf_Integration_With_Salesforce_Onboarding {
	...
	public function __construct() {
		...
		if ( defined( 'MWB_GF_SF_ONBOARD_PLUGIN_NAME' ) ) {
			self::$plugin_name = MWB_GF_SF_ONBOARD_PLUGIN_NAME;
		}
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', MWB_GF_INTEGRATION_WITH_SALESFORCE_URL . 'admin/css/makewebbetter-onboarding-admin.css', array(), '1.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', MWB_GF_INTEGRATION_WITH_SALESFORCE_URL . 'packages/select2/select2.min.css', array(), '1.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', MWB_GF_INTEGRATION_WITH_SALESFORCE_URL . 'admin/js/makewebbetter-onboarding-admin.js', array( 'jquery' ), '1.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', MWB_GF_INTEGRATION_WITH_SALESFORCE_URL . 'packages/select2/select2.min.js', array( 'jquery' ), '1.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once MWB_GF_INTEGRATION_WITH_SALESFORCE_DIRPATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) { // phpcs:ignore
			require_once MWB_GF_INTEGRATION_WITH_SALESFORCE_DIRPATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'mwb-gf-integration-with-salesforce' ),
				'type'        => 'radio',
			...
			wp_rand() => array(
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'mwb-gf-integration-with-salesforce' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			wp_rand() => array(
				'id'          => 'onboard-email',
				'label'       => esc_html__( 'What is the best email address to contact you?', 'mwb-gf-integration-with-salesforce' ),
				'type'        => 'email',
				'name'        => 'email',
			...
			wp_rand() => array(
				'id'          => 'onboard-number',
				'label'       => esc_html__( 'What is your contact number?', 'mwb-gf-integration-with-salesforce' ),
				'type'        => 'text',
				'name'        => 'phone',
				'value'       => '',
				'required'    => 'yes',
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'mwb-gf-integration-with-salesforce' );
			$errors      = $response;
