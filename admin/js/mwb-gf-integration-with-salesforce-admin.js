...
jQuery( document ).ready( function() {
	...
	const notice       = mwb_gf_ajax_data.notice;
		...
		switch( intMethod ) {
			case 'api':
				Swal.fire( {
					...
					'<li>Go to App Manager in Salesforce and Choose "New Connected App"</li>' +
					'<li>Enter application name and check "Enable OAuth Settings"</li>' +
					'<li>Enter ' + adminUrl + ' callback URL</li>' +
					'<li>Select Access & manage your data(api) & Perform requests on your behalf at any time (refresh_token, offline_access) as OAuth Scopes</li>' +
					'<li>Save app, Copy Consumer Key & Secret Key.</li>' +
					'</ul>' + 
			...
			case 'web':
				Swal.fire( {
					title: 'How to get the Webto keys?',
					html:
					'<ul class="mwb_list_points">' +
					'<li>In salesforce Go to Company Information -> Salesforce.com Organization ID</li>' +
					'<li>Enter your salesforce domain with https( For e.g: https://example.salesforce.com ) in Salesforce Domain field</li>' +
					'</ul>' + 
					'<img style=" width: 100%; border: 2px solid;" src="' + webtoImg + '">',
					allowOutsideClick : false,
					showClass: {
						popup: 'animate__animated animate__backInDown'
					},
	...
	const manageAuthForm = ( method = false ) => {
		...
		let form   = jQuery( '#mwb_sf_gf_account_form' );
		if ( false == method ) {
			method = jQuery( 'input[name="mwb_account[integration_method]"]:checked' ).val();
		}
		
		if ( form.length > 0 ) {
			let api_fields = form.find( 'tr.mwb-api-fields' );
			let web_fields = form.find( 'tr.mwb-web-fields' );

			api_fields.css( 'display', 'none' );
			web_fields.css( 'display', 'none' );
		

			if ( method ) {
				switch( method ) {
					case 'api':
						jQuery( '.mwb-sf-gf-use-custom-app' ).slideDown(200);
						if ( jQuery( '#mwb-' + crm + '-gf_enable_own_app' ).prop( 'checked' ) ) {
							api_fields.slideDown(200);
						}
						break;

					case 'web':
						web_fields.slideDown(200);
						jQuery( '.mwb-sf-gf-use-custom-app' ).slideUp(200);
						break;
	...
	jQuery( document ).on( 'click', '.trigger-setup-guide', function() {
		intMethod =  jQuery( 'input[name="mwb_account[integration_method]"]:checked' ).val();
	...
	jQuery( 'input[name="mwb_account[integration_method]"]' ).on( 'change', function() {
		intMethod = this.value;
		manageAuthForm( this.value );
	} );

	/*Custom App select */
	let custom_app = jQuery( '#mwb-' + crm + '-gf_enable_own_app' ).prop( 'checked' ) ? 'yes' : 'no';
	if ( 'yes' == custom_app ) {
		jQuery( '.mwb-api-fields' ).slideDown( 200 );
	} else {
		jQuery( '.mwb-api-fields' ).slideUp( 200 );
	}

	jQuery( '#mwb-' + crm + '-gf_enable_own_app' ).on( 'change', function(){
	...
	jQuery( '#mwb-' + crm + '-gf-authorize-button' ).on( 'click', function( e ) {
		...
		let custom_app      = jQuery( '#mwb-' + crm + '-gf_enable_own_app' ).prop( 'checked' ) ? 'yes' : 'no';
		let enviornment     = jQuery( '#mwb-' + crm + '-gf-environment' ).val();
		let method          = jQuery( 'input[name="mwb_account[integration_method]"]:checked' ).val();	
		let consumer_key    = jQuery( '#mwb-' + crm + '-gf-consumer-key' ).val();
		let consumer_secret = jQuery( '#mwb-' + crm + '-gf-consumer-secret' ).val();
		let orgid           = jQuery( '#mwb-' + crm + '-gf-orgid' ).val();
		let domain          = jQuery( '#mwb-' + crm + '-gf-domain' ). val();

		if ( method.length == 0 ) {
			triggerError( 'Please select an Integration method.' );
			return;
		}
		...
		switch( method ) {
			...
			case 'web':
				if ( orgid.length == 0 ) {
					triggerError( 'Salesforce Org ID is a required field.' );
					return;
				} else {
					href = updateQueryStringParameter( href, 'orgid', orgid );
		...
		href = updateQueryStringParameter( href, 'enviornment', enviornment );
		href = updateQueryStringParameter( href, 'custom_app', custom_app );
		...
		result.then( ( response ) => {
			...
			if ( true == response.status ) {
				if ( true == response.data.success ) {
					...
					jQuery( '#mwb-' + crm + '_gf-token-notice' ).text( response.data.token_message );
				}
			} else {
				triggerError( response.data.msg );
			}
	...
	/* Reauthorize salesforce account */
	...
	jQuery( '#mwb-' + crm + '-refresh-fields' ).on( 'click', function() {
		...
		if ( '-1' == module ) {
			triggerError( 'Please select a Salesforce Object first' );
			return;
		}
	...
	jQuery( document ).on( 'click' , '#add-new-field-btn', function( e ) {
		...
		if( '-1' == object || '-1' == field ) {
			triggerInfo( 'Select a valid Salesforce object and its corresponding field' );
			return;
		}
